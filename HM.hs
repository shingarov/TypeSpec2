...
setv i v = M(\(n,t)->((),(n,updateT i (Yes v) t)))
...
unify x y = indir x `bind` \x'->
	    ...
	    case (x',y') of
	      (TVar m,TVar n) ->
	        if m==n then unit () else setv m (TVar n)
	      (TVar m,t) -> setv m t
	      (t,TVar n) -> setv n t
	      (TFun a b,TFun c d) -> 
	        unify a c `andThen` unify b d
	      (TInt,TInt) -> unit ()
...
tc g (App a b) t = fresh `bind` \s->
                   tc g a (TFun s t) `andThen`
		   tc g b s
tc g (Lam s e) t = fresh `bind` \x->
                   fresh `bind` \y->
...
main = interact (\_ -> run (fresh `bind` \t->
                            tc [] (bm 1000) t `andThen`
			    inst t `bind` \t'->
			    unit (show t')))
